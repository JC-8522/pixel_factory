...
const debugLogPath = path.join(outDir, "conversation-focus-flow.debug.log");
const rawScenario = process.env.PIXEL_FACTORY_AUTOMATION_SCENARIO?.trim();
const scenario =
  rawScenario === "approval" || rawScenario === "attachment" || rawScenario === "controls"
    ? rawScenario
    : "completed";
...
const userDataDir = path.join(outDir, "tmp-runtime", `focus-capture-${Date.now()}`);
const reportPath = path.join(outDir, `${screenshotPrefix}-report.json`);
const stdoutLogPath = path.join(outDir, `${screenshotPrefix}-electron.stdout.log`);
const stderrLogPath = path.join(outDir, `${screenshotPrefix}-electron.stderr.log`);
...
await appendFile(
  debugLogPath,
  `[${new Date().toISOString()}] scenario=${scenario} electronPath=${electronPath} reportPath=${reportPath}\n`,
  "utf8"
);
...
const child = spawn(electronPath, ["--disable-gpu", "--use-angle=swiftshader", process.cwd()], {
  ...
  env: {
    ...process.env,
    PIXEL_FACTORY_AUTOMATION_MODE: "capture_conversation_workspace",
    PIXEL_FACTORY_AUTOMATION_OUT_DIR: outDir,
    PIXEL_FACTORY_AUTOMATION_USER_DATA: userDataDir,
    PIXEL_FACTORY_AUTOMATION_SCENARIO: scenario
  },
